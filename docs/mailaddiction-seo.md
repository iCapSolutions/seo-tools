...
### Step 7 — Regenerate sitemap with policy controls

Use the crawler in `seo-tools/scripts/sitemap_crawl.py` to regenerate sitemap output
after internal-link or content-cluster changes.

```sh
python3 /path/to/seo-tools/scripts/sitemap_crawl.py \
  https://www.mailaddiction.com/ \
  -o /path/to/mailaddiction/templates/sitemap.xml \
  -v
```

Targeted-cluster example (Articles + Resources emphasis):

```sh
python3 /path/to/seo-tools/scripts/sitemap_crawl.py \
  https://www.mailaddiction.com/ \
  -o /path/to/mailaddiction/templates/sitemap.xml \
  --include-prefix /articles \
  --include-prefix /articles.html \
  --include-prefix /resources \
  --include-prefix /resources.html \
  --exclude-prefix /secure \
  -v
```

Policy behavior:
- default excludes: `/secure/`, `/manager/`, `/admin/`, `/cgi-bin/`, `/sitemap_ez`
- robots `noindex` pages are excluded by default
- use `--include-prefix` / `--exclude-prefix` to shape emitted URL sets deterministically
- use `--include-noindex` only when intentionally generating non-indexable URL output
...
- `templates/sitemap.xml` is generated by `seo-tools/scripts/sitemap_crawl.py`
  and should be treated as generated output (don't hand-edit)
- `generator/data/sitemap.xml` belongs to legacy generator flow and remains
